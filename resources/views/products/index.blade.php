    ...
    <div class="container p-4 mx-auto">
        <div class="d-flex">
            ...
            <div class="page-title-icon">
                <i class="fas fa-box-open icon-gradient bg-mean-fruit"></i>
            </div>
        </div>
        @can('product-create')
            <a href="{{ route('products.create') }}" class="px-4 py-2 font-bold text-white bg-blue-500 rounded hover:bg-blue-700">
                + Add New Product
            </a>
        @endcan
            ...
            @foreach ($products as $data)
                <div class="p-4 bg-white rounded shadow">
                    ...
                    <p class="text-gray-600">status: <span
                            class="font-bold {{ $data['status'] ? 'text-green-500' : 'text-red-500' }}">{{ $data['status'] ? 'Active' : 'Inactive' }}</span>
                    </p>
